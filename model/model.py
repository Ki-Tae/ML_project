...
with tf.Session() as sess:
    ...
    for epoch in range(max_epoch):
        ...
        minimize.run(feed_dict={inputs:np.transpose(X_inputs), outputs:E_outputs.reshape([-1,1])})
        if (epoch+1) % 100 == 0:
            ...
            print(epoch+1, sess.run(loss, feed_dict={inputs:np.transpose(X_inputs), outputs:E_outputs.reshape([-1,1])}))
